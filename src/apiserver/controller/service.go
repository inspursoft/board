...
const (
	...
	deploymentTestFilename = "deploymenttest.yaml"
	serviceTestFilename    = "servicetest.yaml"
...
func (p *ServiceController) DeployServiceTestAction() {
	...
	//Build deployment yaml file
	err = service.BuildTestDeploymentYaml(reqServiceConfig)
	if err != nil {
		p.internalError(err)
		return
	}
	...
	//Build service yaml file
	err = service.BuildTestServiceYaml(reqServiceConfig)
	if err != nil {
		p.internalError(err)
		return
	}
	...
	// Push deployment to jenkins
	pushobject.FileName = deploymentTestFilename
	pushobject.JobName = serviceProcess
	pushobject.Value = filepath.Join(reqServiceConfig.ProjectName, strconv.Itoa(serviceId))
	pushobject.Message = fmt.Sprintf("Create deployment for project %s service %d",
		reqServiceConfig.ProjectName, reqServiceConfig.ServiceID)
	...
	logs.Info("Internal push test deployment object: %d %s", ret, msg)

	//TODO: If fail to create deployment, should not continue to create service

	//Push service to jenkins
	pushobject.FileName = serviceTestFilename
	pushobject.Message = fmt.Sprintf("Create test service for project %s service %d",
		reqServiceConfig.ProjectName, reqServiceConfig.ServiceID)
	pushobject.Extras = filepath.Join(kubeMasterURL(), serviceAPI, serviceNamespace, "/services")
	...
	logs.Info("Internal push test service object: %d %s", ret, msg)

	// Update service status in database
	serviceName := reqServiceConfig.ServiceYaml.Name + service.Test
	updateService := model.ServiceStatus{ID: int64(serviceID), Status: running,
		Name: serviceName}
	_, err = service.UpdateService(updateService, "name", "status")
	if err != nil {
