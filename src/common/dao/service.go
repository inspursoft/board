...
func generateServiceStatusSQL(query model.ServiceStatus, userID int64) (string, []interface{}) {
	sql := `select distinct s.id, s.name, s.project_id, s.project_name, u.username as owner_name, s.owner_id, s.creation_time, s.update_time, s.status, s.type, s.public, s.source, p.pull_request_count,
